...
class Validate{
	...
	public function run(){
		foreach($this->rules as $element => $value){
			if($value['required'] == true && trim($this->source[$element])==null){
				$this->errors[$element] = 'Không được rỗng.';
			}else{
				switch ($value['type']) {
					case 'int':
	...
	private function validateString($element, $min = 0, $max = 0){
		...
		if($length < $min) {
			$this->errors[$element] = "'" . $this->source[$element] . "' không hợp lệ. Độ dài phải từ $min đến $max ký tự";
		}elseif($length > $max){
			$this->errors[$element] = "'" . $this->source[$element] . "' ikhông hợp lệ. Độ dài phải từ $min đến $max ký tự";
	...
	public function showErrors(){
		...
		if(!empty($this->errors)){
			$xhtml .= '<ul style="list-style:inside;color:red;">';
